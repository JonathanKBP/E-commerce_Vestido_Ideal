...
CREATE TABLE `loja`.`tb_itens_cesta` (
    ...
    FOREIGN KEY(`fk_id_cesta`) REFERENCES `tb_cesta`(`pk_id_cesta`) on delete cascade,
	FOREIGN KEY(`fk_id_produto`) REFERENCES `tb_roupa`(`pk_id_produto`) on delete cascade,
...
select * from tb_cliente where pk_id_cliente='2';
